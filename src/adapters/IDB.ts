...
import BaseAdapter, { ListParameters } from "./base";
 ...
 */
function findIndexedField(filters: Object): string {
 ...
 *
 * @param  {IDBObjectStore}   store      The IDB store.
 ...
 * @return {IDBRequest}
 */
function createListRequest(store: IDBObjectStore, indexField: string | undefined, value: any, filters: Object, done: (x: any[]) => any): IDBRequest {
  if (!indexField) {
    // Get all records.
...
export default class IDB extends BaseAdapter {
  ...
  open(): Promise<IDB> {
    ...
    return new Promise<IDB>((resolve, reject) => {
      ...
      request.onupgradeneeded = event => {
        ...
        const collStore: IDBObjectStore = db.createObjectStore(this.dbname, {
          keyPath: "id"
        });
        ...
        // Metadata store
        const metaStore: IDBObjectStore = db.createObjectStore("__meta__", {
          keyPath: "name"
        });
   ...
   * @return {Object}
   */
  prepare(mode: string | undefined = undefined, name: string | null = null) {
    if (!this._db) {
      throw ("prepare(..) called before open(..)");
    }
   ...
   * @return {Promise}
   */
  async list(params: ListParameters = { filters: {} }) {
    const {filters} = params;
    const indexField = findIndexedField(filters as Object);
    const value = (filters as Object)[indexField];
    try {
      await this.open();
      const results = await new Promise<any[]>((resolve, reject) => {
        let results: any[] = [];
        // If `indexField` was used already, don't filter again.
        const remainingFilters = omitKeys((filters as Object), [indexField]);
 ...
 *
 * @param  {IDBObjectStore} store     The IndexedDB database store.
 * @param  {Array}          preloaded The list of records to make available to
 *                                    get() (default: []).
 * @return {Object}
 */
function transactionProxy(store: IDBObjectStore, preloaded = []) {
  return {
    create(record) {
      store.add(record);
    },
